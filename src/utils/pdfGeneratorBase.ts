...
export class BasePDFGenerator {
  ...
  constructor() {
    ...
    this.margin = 10; // Margen de 10mm como en el PDF original
  ...
  protected drawHeader(meta: BasePDFDocumentMeta) {
    ...
    // Left section - Hospital logo (width: 50mm)
    const leftWidth = 50;
    ...
    this.pdf.text(meta.formatoNumero, centerX, headerY + 7, { align: 'center' });
    this.pdf.text(meta.titulo, centerX, headerY + 12, { align: 'center' });
    this.pdf.setFontSize(8);
    this.pdf.text(meta.subtitulo, centerX, headerY + 17, { align: 'center' });
    
    // Right section - Code/Version/Date table (width: 50mm)
    const rightX = this.margin + leftWidth + centerWidth;
    const cellHeight = headerHeight / 3;
    
    // Vertical divider in right section
    this.pdf.line(rightX + 25, headerY, rightX + 25, headerY + headerHeight);
    ...
    this.pdf.text(meta.codigo, rightX + 27, headerY + 5);
    this.pdf.text(meta.version, rightX + 27, headerY + cellHeight + 5);
    this.pdf.text(meta.fecha, rightX + 27, headerY + 2 * cellHeight + 5);
  ...
  protected drawFallbackLogo(headerY: number) {
    this.pdf.setFontSize(9);
    this.pdf.setFont('helvetica', 'bold');
    this.pdf.text('E.S.E', this.margin + 15, headerY + 6);
    this.pdf.text('HOSPITAL', this.margin + 10, headerY + 10);
    this.pdf.text('LA MESA', this.margin + 12, headerY + 14);
    this.pdf.setFontSize(6);
  ...
  protected drawPatientData(data: BasePDFData) {
    ...
    const col1 = 70; // NOMBRE COMPLETO
    const col2 = 15; // SEXO
    const col3 = 40; // FECHA Y HORA
    ...
    this.pdf.text(nameLines, xPos + 1, tableY + 9);
    xPos += col1;
    ...
    this.pdf.setFontSize(6);
    this.pdf.setFont('helvetica', 'bold');
    this.pdf.text('SEXO', xPos + 1, tableY + 5);
    ...
    this.pdf.rect(xPos + 4, sexoY + 1.5, 3, 3);
    if (data.patientData.sexo === 'F') {
      this.pdf.text('X', xPos + 4.5, sexoY + 4);
    }
    this.pdf.text('M', xPos + 9, sexoY + 4);
    this.pdf.rect(xPos + 11, sexoY + 1.5, 3, 3);
    if (data.patientData.sexo === 'M') {
      this.pdf.text('X', xPos + 11.5, sexoY + 4);
    }
    xPos += col2;
    ...
    this.pdf.setFontSize(6);
    this.pdf.setFont('helvetica', 'bold');
    this.pdf.text('FECHA Y HORA', xPos + 1, tableY + 5);
    this.pdf.setFont('helvetica', 'normal');
    this.pdf.setFontSize(7);
    this.pdf.text(data.fechaHora, xPos + 1, tableY + rowHeight + 5);
    xPos += col3;
    ...
    this.pdf.setFont('helvetica', 'normal');
    this.pdf.setFontSize(7);
    this.pdf.text(data.patientData.numeroDocumento, xPos + 1, row2Y + rowHeight + 5);
    xPos += docCol;
    ...
    tipos.forEach((t, idx) => {
      ...
      this.pdf.rect(tipoX, row2Y + rowHeight + 2, 3, 3);
      if (isSelected) {
        this.pdf.text('X', tipoX + 0.5, row2Y + rowHeight + 4.5);
      }
      this.pdf.text(t, tipoX + 4, row2Y + rowHeight + 4.5);
      tipoX += (idx < 4 ? 6 : 0);
    });
    xPos += tipoCol;
    ...
    this.pdf.setFont('helvetica', 'normal');
    this.pdf.setFontSize(7);
    this.pdf.text(`${data.patientData.edad}`, xPos + 1, row2Y + rowHeight + 5);
    xPos += edadCol;
    ...
    this.pdf.text(epsLines, xPos + 1, row2Y + rowHeight + 4);
    xPos += eapbCol;
    ...
    regimenes.forEach((r) => {
      ...
      this.pdf.rect(regX, row2Y + rowHeight + 2, 3, 3);
      if (isSelected) {
        this.pdf.text('X', regX + 0.5, row2Y + rowHeight + 4.5);
      }
      this.pdf.text(r.label, regX + 4, row2Y + rowHeight + 4.5);
      regX += 10;
    });
    
    this.currentY = row2Y + rowHeight * 2 + 2;
  }
  ...
  protected drawGuardianData(guardian: BasePDFGuardianData) {
    ...
    this.pdf.setFont('helvetica', 'normal');
    this.pdf.setFontSize(7);
    this.pdf.text(guardian.nombreCompleto, this.margin + 32, tableY + 4);
    this.pdf.text(guardian.documento, this.margin + this.contentWidth * 0.6 + 22, tableY + 4);
  ...
  protected drawProcedureSection(procedures: BasePDFProcedureItem[]) {
    ...
    for (const item of procedures) {
      ...
      if (this.currentY + rowHeight > this.pageHeight - 25) {
        this.drawFooter();
        this.pdf.addPage();
        this.currentY = this.margin;
        // Don't redraw full header on continuation pages, just minimal header
      }
  ...
  protected drawSignatureSection(data: BasePDFData) {
    ...
    if (data.patientSignature && 
      ...
      try {
        this.pdf.addImage(data.patientSignature, 'PNG', this.margin + 2, this.currentY + 2, colWidth - 4, signatureHeight - 10);
      } catch (error) {
    ...
    if (data.professionalData.firma && 
      ...
      try {
        this.pdf.addImage(data.professionalData.firma, 'PNG', this.margin + 2 * colWidth + 2, this.currentY + 2, colWidth - 4, signatureHeight - 10);
      } catch (error) {
    ...
    // Signature labels with underlines
    ...
    this.pdf.text(`Documento: ${data.patientData.numeroDocumento}`, this.margin + 2, labelY + 7);
    
    // Representative signature label (middle column)
    this.pdf.text('Firma Representante legal:', this.margin + colWidth + 2, labelY + 3);
    if (data.guardianData) {
      this.pdf.text(`Documento: ${data.guardianData.documento}`, this.margin + colWidth + 2, labelY + 7);
    } else {
      this.pdf.text('Documento: ________________', this.margin + colWidth + 2, labelY + 7);
    }
    
    // Professional signature label
    this.pdf.text('Nombre y documento de quien toma el', this.margin + 2 * colWidth + 2, labelY + 3);
    this.pdf.text('consentimiento:', this.margin + 2 * colWidth + 2, labelY + 7);
    this.pdf.setFontSize(5);
    this.pdf.text(data.professionalData.nombreCompleto, this.margin + 2 * colWidth + 2, labelY + 11);
    this.pdf.text(`Doc: ${data.professionalData.documento}`, this.margin + 2 * colWidth + 2, labelY + 14);
    ...
    if (data.patientPhoto && 
      ...
      try {
        ...
        this.currentY += 3;
        this.pdf.addImage(data.patientPhoto, 'JPEG', this.margin, this.currentY, 25, 20);
        this.currentY += 22;
      } catch (error) {
  ...
  protected drawWithdrawalSection(data: BasePDFData) {
    ...
    const dateText = `En manifestación de desistimiento firmo/pongo mi huella en este documento a los ${day} días del mes de ${month} de ${year}.`;
    const dateLines = this.pdf.splitTextToSize(dateText, this.contentWidth - 4);
    this.pdf.text(dateLines, this.margin + 2, this.currentY);
    ...
    this.pdf.text(`Documento: ${data.patientData.numeroDocumento}`, this.margin + 2, this.currentY + 7);
    
    this.pdf.text('Firma Representante legal:', this.margin + colWidth + 2, this.currentY + 3);
    this.pdf.text('Documento: ________________', this.margin + colWidth + 2, this.currentY + 7);
    
    this.pdf.text('Nombre y documento de quien toma el', this.margin + 2 * colWidth + 2, this.currentY + 3);
    this.pdf.text('desistimiento:', this.margin + 2 * colWidth + 2, this.currentY + 7);
