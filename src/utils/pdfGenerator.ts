...
export class ConsentPDFGenerator {
  ...
  private renderHeader(): void {
    ...
    this.pdf.text('E.S.E. Hospital La Mesa Pedro Leon Alvarez Diaz', this.margin + 20, this.currentY + 18, { align: 'center' });
    ...
    this.pdf.setFont("helvetica", "bold");
    this.pdf.text('E.S.E. HOSPITAL LA MESA', centerX + centerWidth/2, this.currentY + 8, { align: 'center' });
    this.pdf.setFontSize(9);
    this.pdf.text('PEDRO LEON ALVAREZ DIAZ', centerX + centerWidth/2, this.currentY + 13, { align: 'center' });
    this.pdf.setFontSize(10);
    this.pdf.text('CONSENTIMIENTO INFORMADO TOMA DE', centerX + centerWidth/2, this.currentY + 18, { align: 'center' });
    this.pdf.text('MUESTRAS LABORATORIO', centerX + centerWidth/2, this.currentY + 23, { align: 'center' });
    ...
    this.pdf.text('1 de 3', infoX + 35, this.currentY + 4, { align: 'right' });
    this.pdf.text('Versión:', infoX + 2, this.currentY + 8);
    this.pdf.text('08', infoX + 35, this.currentY + 8, { align: 'right' });
    ...
    this.pdf.text('1203SUBCIE-F73', infoX + 35, this.currentY + 16, { align: 'right' });
  ...
  private renderPatientData(data: ConsentPDFData): void {
    ...
    const isHospital = data.patientData.centroSalud === "Hospital La Mesa Pedro Leon Alvarez Diaz";
    this.pdf.circle(this.margin + 3, this.currentY, 2);
    if (isHospital) this.pdf.circle(this.margin + 3, this.currentY, 1, 'F');
    this.pdf.text('HOSPITAL LA MESA PEDRO LEON ALVAREZ DIAZ', this.margin + 8, this.currentY + 1);
...
Seleccione el (los) Procedimiento(s) que se va(n) a realizar:`;
  ...
  private renderProceduresTable(data: ConsentPDFData): void {
    ...
    const allProcedures = [
      {
        ...
        descripcion: "La venopunción es la técnica por la cual se perfora una vena por vía transcutánea con una aguja o catéter; el objetivo de este procedimiento es la extracción de sangre para su posterior análisis.",
      ...
      },
      {
        ...
        descripcion: "Se tomará muestra de la secreción para examen directo, previa colocación o no de espéculo vaginal (depende del caso) muestra se toma con ayuda de un aplicador",
        riesgos: "No existe ningún riesgo identificado a la toma de la muestra, incluso si se encuentra embarazada actualmente. Cuando el cuello del útero se encuentra inflamado, en ocasiones se presenta escaso sangrado vaginal (manchado), que cede solo y no requiere tratamiento.",
        beneficios: "Orientación diagnóstica de su estado de salud. Seguimiento a tratamiento de su enfermedad",
      ...
      },
      {
        ...
        descripcion: "El examen de secreción uretral se realiza para determinar la presencia de microorganismos bacterianos en la uretra (zona de las vías urinarias masculina)",
        riesgos: "Puede sentir presión o estar doliente el hisopo toca la uretra",
        beneficios: "Orientación diagnóstica de su estado de salud. Seguimiento a tratamiento de su enfermedad",
        alternativas: "Informar a su médico, con el fin de enviar tratamiento sin conocer diagnóstico",
        implicaciones: "No hay implicaciones evidenciadas en literatura y experiencia"
      },
      {
        ...
        descripcion: "Consiste en la determinación de los niveles de Glucosa en sangre en una muestra basal (en ayunas), posterior a esto se le administrará bajo una carga de glucosa estipulada por el médico (tubo). Por parte del personal del laboratorio se le asignarán los tiempos en los cuales usted debe regresar al laboratorio para la toma de la muestra.",
        riesgos: "Náuseas, mareos, malestar general, vómitos.",
        beneficios: "Diagnóstico preciso de alteraciones en el metabolismo de la glucosa, permitiendo intervención temprana.",
        alternativas: "Hemoglobina glucosilada (HbA1c), glucosa plasmática en ayunas, prueba de glucosa postprandial.",
        implicaciones: "Requiere ayuno previo, duración de 2 a 3 horas en el laboratorio, cooperación del paciente y condiciones adecuadas para evitar falsos resultados."
      },
      {
        id: "puncion_arterial",
        nombre: "Punción Arterial",
        descripcion: "Procedimiento para obtener una muestra de sangre directamente de una arteria, generalmente la arteria radial o femoral, para medir gases en sangre y otros parámetros. Se usa comúnmente en situaciones críticas para evaluar la función pulmonar y el equilibrio ácido-base.",
    ...
    allProcedures.forEach((proc, index) => {
      const isSelected = data.selectedProcedures.some(selected => selected.id === proc.id);
      
      // Calculate required height for this row
      const maxLines = Math.max(
        this.pdf.splitTextToSize(proc.descripcion, colWidths[2] - 4).length,
        this.pdf.splitTextToSize(proc.riesgos, colWidths[3] - 4).length,
        this.pdf.splitTextToSize(proc.beneficios, colWidths[4] - 4).length,
        this.pdf.splitTextToSize(proc.alternativas, colWidths[5] - 4).length,
        this.pdf.splitTextToSize(proc.implicaciones, colWidths[6] - 4).length
      );
      
      const currentRowHeight = Math.max(rowHeight, maxLines * 3 + 4);
      
      // Check if we need a new page
      if (this.currentY + currentRowHeight > this.pageHeight - this.margin - 15) {
        this.pdf.addPage();
        this.currentY = this.margin;
        ...
        headers.forEach((header, headerIndex) => {
          this.pdf.rect(x, this.currentY, colWidths[headerIndex], headerHeight, 'FD');
          this.pdf.text(header, x + 2, this.currentY + 8);
      ...
      this.pdf.text(proc.nombre, x + 2, this.currentY + 6);
      x += colWidths[1];
      
      // Description
      this.pdf.rect(x, this.currentY, colWidths[2], currentRowHeight);
      const descLines = this.pdf.splitTextToSize(proc.descripcion, colWidths[2] - 4);
      ...
      const riskLines = this.pdf.splitTextToSize(proc.riesgos, colWidths[3] - 4);
      ...
      const benefitLines = this.pdf.splitTextToSize(proc.beneficios, colWidths[4] - 4);
      ...
      const altLines = this.pdf.splitTextToSize(proc.alternativas, colWidths[5] - 4);
      ...
      const implLines = this.pdf.splitTextToSize(proc.implicaciones, colWidths[6] - 4);
...
voluntaria.`;
  ...
  private renderSignatureSection(data: ConsentPDFData): void {
    ...
    // Patient photo next to signature
    const photoWidth = 35;
