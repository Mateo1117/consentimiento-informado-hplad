...
export const CameraCapture = forwardRef<CameraCaptureRef, CameraCaptureProps>(({
  ...
  const getCapturedPhoto = useCallback(() => {
    logger.debug("Getting captured photo", {
  ...
  return <Card className="w-full">
        ...
        <CardContent>
          <div className="relative bg-muted rounded-lg overflow-hidden w-full h-64 min-h-[280px]">
            {capturedPhoto ? <img src={capturedPhoto} alt="Foto capturada" className="w-full h-full object-cover" /> : <>
                <video ref={videoRef} autoPlay playsInline muted className={`w-full h-full object-cover ${!isCameraActive ? 'hidden' : ''}`} />
                {!isCameraActive && <div className="absolute inset-0 flex items-center justify-center">
          ...
          <div className="flex gap-2 justify-center">
            {!capturedPhoto ? <>
                {!isCameraActive ? <Button onClick={startCamera} variant="outline">
                    <Camera className="h-4 w-4 mr-2" />
                    Activar Cámara
                  </Button> : <Button onClick={capturePhoto} variant="default">
                    <Camera className="h-4 w-4 mr-2" />
                    Capturar Foto
                  </Button>}
