-- Verificar y crear/corregir las funciones RPC para firma móvil

-- Eliminar funciones existentes si existen (para recrearlas)
...
-- Función para obtener consentimiento por token con seguridad
...
-- Función para firmar consentimiento por token con seguridad
...
BEGIN
  ...
  IF v_consent_id IS NULL THEN
    RAISE EXCEPTION 'Token de consentimiento no válido o expirado';
  END IF;
...
CREATE INDEX IF NOT EXISTS idx_consent_access_logs_token ON consent_access_logs(share_token);
CREATE INDEX IF NOT EXISTS idx_consent_signature_logs_consent_id ON consent_signature_logs(consent_id);

-- Verificar que la tabla consents tiene las columnas necesarias
DO $$
BEGIN
