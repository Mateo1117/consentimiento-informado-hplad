...
const corsHeaders = {
  ...
  // Importante: supabase-js agrega headers x-supabase-client-* en el navegador.
  // Si no se permiten, el preflight CORS falla y la función ni siquiera se ejecuta.
...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log("🔗 Enviando al webhook (POST)", { 
      url: WEBHOOK_URL,
      payloadSize: JSON.stringify(webhookPayload).length 
    });

    // Enviar por POST con JSON body en lugar de GET con query params
